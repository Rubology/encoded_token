...
module LegacyEncodingTests
  class DecodeTest < MinitestBase
    ...
    describe ':decode' do
      ...
      let(:decoder)       { EncodedToken::Encoder::LegacyDecoder }
      let(:uuid)          { SecureRandom.uuid }
      let(:encoded_token) { EncodedToken::Encoder::LegacyEncoder.encode(uuid) }
      
      context "on success" do
        it "decodes a token" do
          decoded_token = decoder.decode(encoded_token)
          assert decoded_token
          assert_equal uuid, decoded_token
        end
        ...
        it "tries with each decoding ciphers" do
          ...
          orig_token = EncodedToken::Encoder::LegacyEncoder.encode(uuid)
          ...
          assert_equal uuid, EncodedToken::Encoder::LegacyDecoder.decode(orig_token)
        end
      end # on success
